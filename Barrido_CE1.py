...
ce1_values = np.random.uniform(5.5e-12, 6e-12, 10)
...
with open("datos_ndf.csv", "w", newline='') as csvfile:
    ...
    writer.writerow(["Ce1 (F)", "Frecuencia (Hz)", "NDF_Re", "NDF_Im"])

    # Iteramos sobre todos los valores del condensador Ce1
    for ce1_val in ce1_values:
        # Asignamos el valor actual al parámetro "C" del componente c7
        ...
        try:
            ...
            # Guardamos los datos en el CSV: Ce1, frecuencia, Re(NDF), Im(NDF)
            for f, re, im in zip(freqs, vals_re, vals_im):
                writer.writerow([ce1_val, f, re, im])

        except Exception as e:
            # Capturamos e imprimimos errores, por ejemplo si falla la simulación
            print(f"Error en Ce1 = {ce1_val*1e12:.2f} pF: {e}")
...
# Extraemos las frecuencias únicas y los valores únicos de Ce1
frequencies = sorted(df["Frecuencia (Hz)"].unique())
ce1_vals = df["Ce1 (F)"].unique()

# Inicializamos una figura polar (tipo carta de Smith modificada)
plt.figure(figsize=(7, 7))
ax = plt.subplot(111, projection="polar")

# Recorremos cada valor de Ce1 y trazamos su trayectoria de NDF en el plano polar
for ce1 in ce1_vals:
    subset = df[df["Ce1 (F)"] == ce1]  # Filtramos datos para este Ce1
...
# Agrupamos los datos por frecuencia y calculamos el valor medio (promedio sobre Ce1)
...
########## AF-SPARAMS #############
# Archivo de salida
with open("datos_ce1.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Ce1 (F)", "Frecuencia (Hz)", "|S11| (dB)", "|S12| (dB)", "|S21| (dB)", "|S22| (dB)"])
    ...
    for ce1_val in ce1_values:
        ...
        for f, v11, v12, v21, v22 in zip(fs, s11, s12, s21, s22):
            writer.writerow([ce1_val, f, v11, v12, v21, v22])


# Cargar archivo CSV
df = pd.read_csv("datos_ce1.csv")

# Obtener todos los valores únicos de Ce1
ce1_vals = df["Ce1 (F)"].unique()

if len(ce1_vals) == 0:
    raise ValueError("No se encontraron valores de Ce1 en el archivo.")

# Usar el primer valor disponible
ce1_target = ce1_vals[0]
subset = df[df["Ce1 (F)"] == ce1_target]
...
plt.title("af_sparams (barrido en Ce1)")
...
ce1_vals = sorted(df["Ce1 (F)"].unique())
...
plt.suptitle("af_sparams (barrido en Ce1)")
...
with open("datos_pz.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Ce1 (F)", "Frecuencia (Hz)", "DB(|Vac(ACCS.I1)|)", "ANG(|Vac(ACCS.I1)|)"])
    ...
    for ce1_val in ce1_values:
        ...
        for f, vdb, vang in zip(fs, db, ang):
            writer.writerow([ce1_val, f, vdb, vang])

# Cargar el CSV
df = pd.read_csv("datos_pz.csv")
df["Frecuencia (GHz)"] = df["Frecuencia (Hz)"] / 1e9

# Obtener valores únicos de Ce1
ce1_vals = sorted(df["Ce1 (F)"].unique())
...
ax1.set_title("Active Feedback (barrido en Ce1)")
ax1.grid(True)
ax1.legend()
...
with open("datos_tx.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Ce1 (F)", "Frecuencia (Hz)", "|S21| (dB)", "CSRR_txon:|S21| (dB)"])
    ...
    for ce1_val in ce1_values:
        ...
        for f, v21, CSRR_s21 in zip(fs, s21, CSRR_s21):
            writer.writerow([ce1_val, f, v21, CSRR_s21])
...
# Agrupar por las simulaciones distintas (supongo Ce1 como la variable barrida)
ce1_vals = sorted(df["Ce1 (F)"].unique())
...
plt.xlabel("Frecuencia (GHz)")
plt.ylabel("|S21| (dB)")
plt.title("active_sensor_txon_sparams (barrido en Ce1)")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
with open("datos_rx.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Ce1 (F)", "Frecuencia (Hz)", "|S11| (dB)", "CSRR_rxon:|S11| (dB)"])
    ...
    for ce1_val in ce1_values:
        ...
        for f, v11, CSRR_s11 in zip(fs, s11, CSRR_s11):
            writer.writerow([ce1_val, f, v11, CSRR_s11])
...
# Obtener los valores de Ce1 únicos
ce1_vals = sorted(df["Ce1 (F)"].unique())
...
plt.xlabel("Frecuencia (GHz)")
plt.ylabel("|S11| (dB)")
plt.title("active_sensor_rxon_sparams (barrido en Ce1)")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
