...
# Barrido de valores de ce6
ce6_values = np.random.uniform(0.75e-12, 1.25e-12, 15)
...
with open("datos_ndf.csv", "w", newline='') as csvfile:
    ...
    writer.writerow(["ce6 (F)", "Frecuencia (Hz)", "NDF_Re", "NDF_Im"])

    # Iteramos sobre todos los valores del condensador ce6
    for ce6_val in ce6_values:
        # Asignamos el valor actual al parámetro "C" del componente c6
        ...
        try:
            ...
            # Guardamos los datos en el CSV: ce6, frecuencia, Re(NDF), Im(NDF)
            for f, re, im in zip(freqs, vals_re, vals_im):
                writer.writerow([ce6_val, f, re, im])

        except Exception as e:
            # Capturamos e imprimimos errores, por ejemplo si falla la simulación
            print(f"Error en ce6 = {ce6_val*1e12:.2f} pF: {e}")
...
# Extraemos las frecuencias únicas y los valores únicos de ce6
frequencies = sorted(df["Frecuencia (Hz)"].unique())
ce6_vals = df["ce6 (F)"].unique()

# Inicializamos una figura polar (tipo carta de Smith modificada)
plt.figure(figsize=(7, 7))
ax = plt.subplot(111, projection="polar")

# Recorremos cada valor de ce6 y trazamos su trayectoria de NDF en el plano polar
for ce6 in ce6_vals:
    subset = df[df["ce6 (F)"] == ce6]  # Filtramos datos para este ce6
...
# Agrupamos los datos por frecuencia y calculamos el valor medio (promedio sobre ce6)
...
########## AF-SPARAMS #############
# Archivo de salida
with open("datos_ce6.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["ce6 (F)", "Frecuencia (Hz)", "|S11| (dB)", "|S12| (dB)", "|S21| (dB)", "|S22| (dB)"])
    ...
    for ce6_val in ce6_values:
        ...
        for f, v11, v12, v21, v22 in zip(fs, s11, s12, s21, s22):
            writer.writerow([ce6_val, f, v11, v12, v21, v22])


# Cargar archivo CSV
df = pd.read_csv("datos_ce6.csv")

# Obtener todos los valores únicos de ce6
ce6_vals = df["ce6 (F)"].unique()

if len(ce6_vals) == 0:
    raise ValueError("No se encontraron valores de ce6 en el archivo.")

# Usar el primer valor disponible
ce6_target = ce6_vals[0]
subset = df[df["ce6 (F)"] == ce6_target]
...
plt.title("af_sparams (barrido en ce6)")
...
ce6_vals = sorted(df["ce6 (F)"].unique())
...
plt.suptitle("af_sparams (barrido en ce6)")
...
with open("datos_pz.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["ce6 (F)", "Frecuencia (Hz)", "DB(|Vac(ACCS.I1)|)", "ANG(|Vac(ACCS.I1)|)"])
    ...
    for ce6_val in ce6_values:
        ...
        for f, vdb, vang in zip(fs, db, ang):
            writer.writerow([ce6_val, f, vdb, vang])

# Cargar el CSV
df = pd.read_csv("datos_pz.csv")
df["Frecuencia (GHz)"] = df["Frecuencia (Hz)"] / 1e9

# Obtener valores únicos de ce6
ce6_vals = sorted(df["ce6 (F)"].unique())
...
ax1.set_title("Active Feedback (barrido en ce6)")
ax1.grid(True)
ax1.legend()
...
with open("datos_tx.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["ce6 (F)", "Frecuencia (Hz)", "|S21| (dB)", "CSRR_txon:|S21| (dB)"])
    ...
    for ce6_val in ce6_values:
        ...
        for f, v21, CSRR_s21 in zip(fs, s21, CSRR_s21):
            writer.writerow([ce6_val, f, v21, CSRR_s21])
...
# Agrupar por las simulaciones distintas (supongo ce6 como la variable barrida)
ce6_vals = sorted(df["ce6 (F)"].unique())
...
plt.xlabel("Frecuencia (GHz)")
plt.ylabel("|S21| (dB)")
plt.title("active_sensor_txon_sparams (barrido en ce6)")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
with open("datos_rx.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["ce6 (F)", "Frecuencia (Hz)", "|S11| (dB)", "CSRR_rxon:|S11| (dB)"])
    ...
    for ce6_val in ce6_values:
        ...
        for f, v11, CSRR_s11 in zip(fs, s11, CSRR_s11):
            writer.writerow([ce6_val, f, v11, CSRR_s11])
...
# Obtener los valores de ce6 únicos
ce6_vals = sorted(df["ce6 (F)"].unique())
...
plt.xlabel("Frecuencia (GHz)")
plt.ylabel("|S11| (dB)")
plt.title("active_sensor_rxon_sparams (barrido en ce6)")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
c6= o.Elements.Add("CAP", 1400, -1600, 0, False)
...
o.Wires.Add(c6.Nodes.Item(2).x, c6.Nodes.Item(2).y, l2.Nodes.Item(1).x, l2.Nodes.Item(1).y)
